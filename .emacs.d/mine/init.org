...
;;     ;; smart-jump-simple-find-referencesのためにagが必要
;;     (leaf ag
;;       :ensure t)))
#+end_src
...
(leaf smart-jump
  ...
  :bind
  ("s-b" . smart-jump-go)
  ("C-s-<left>" . smart-jump-back)
  :config
  (leaf ag
    :doc "smart-jump-simple-find-referencesのためにagが必要"
    :ensure t)
  (leaf dumb-jump
    :ensure t
...
#+begin_src emacs-lisp :lexical no :results silent
  (leaf swift-mode
    ...
    (leaf flycheck-swift
      ...
      (defvar xcode-path (string-trim (shell-command-to-string "xcode-select --print-path")))
      :custom
      (flycheck-swift-target . "arm64-apple-ios-simulator")
      :config
      (setq flycheck-swift-sdk-path (concat xcode-path "/Platforms/iPhoneOS.platform/Developer/SDKs/iPhoneOS10.0.sdk"))))
#+end_src
